...
.controller("manage_projs", function($scope, $project, $state, $utils,
    ...
    $scope.loadPageData = function( pageNo  ){
	    var d =   { itemsPerPage: per , currentPage: pageNo} ;
...
.controller("proj_prop_station", function($scope,  $project, $state, $map, $source ,
	...
	$scope.updataORdel = "updata";
  	...
  	var  pd = {options:"of_proj" , proj_id: $scope.project.id } ,
	...
	$scope.loadPageData = function(pageNo) {
		// 分页加载 采集站数据;
		var d = angular.extend({ currentPage: pageNo }, $sys.pager , pd ) ;
		...
		$source.$system.query( d ).$promise.then( function( resp ){
			...
			// 需要同步的 system
			promise_B = $source.$system.needSync( {uuids: Object.keys(sys_ref) } ).$promise.then( function( respB ){
...
.controller("proj_prop_attr", function($state, $scope,  $project) {
	...
	$scope.commit = function() {
		...
		$project.updataProject(  {projid: $scope.project.id}, $scope.proj, function(resp) {
