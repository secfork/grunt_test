...
angular.module('app.basecontroller', [])
  ...
  .controller("access_signin", function ($scope, $account, $state, $timeout, $localStorage, $sys, $sessionStorage) {
    ...
    $scope.user.companyName = $localStorage.comp_name;
    ...
    $scope.login = function () {
      ...
      $localStorage.comp_name = $scope.user.companyName;
  ...
  .controller("access_signup", function ($scope, $account, $state, $timeout, $localStorage) {

    var delay;
    $scope.isDuplicate = function () {
      delay = $timeout.cancel(delay);

      var n = $.trim($scope.comp.company_name);
      console._log(n);
      if (n) {
        delay = $timeout(
          function () {
            $account.isDupOfAcco({
                comp_name: $scope.comp.company_name
              },
              function (resp) {
                console._log(resp);
                if (resp)
                  $scope.nameok = !resp.ret;
              }
            )
          }, 500
        );
    ...
    // 验证邀请码 激活码;
    $scope.isInviteCode = function () {
      delay = $timeout.cancel(delay);
      delay = $timeout(
        function () {
          $account.isInviteCode({
              invitecode: $scope.comp.invite_code
            },
            function (resp) {
              console._log(resp);
              if (resp)
                $scope.codeok = !resp.ret;
            }
          )
        }, 500
      );
    ...
    $scope.changePic = function ($event) {
      console._log($event);
      var $imgdom = $($event.target).prev("img"),
        d = new Date();
      $imgdom.attr("src", "web/common/identify_img/" + d.getSeconds());
    }
    ...
    $scope.signup = function () {
      $account.signUp(
        //{company_name:$scope.username , invite_code:$scope.password}  ,
        $scope.comp,
        function (resp) {
          if (resp.ret) {
            // 注册成功;
            $localStorage.comp_name = $scope.comp.companyName;
            // $scope.regok = true ;
  ...
  .controller("access_fogpas", function ($scope, $account) {
