...
.controller("dastation_ignore_active", function($scope, $state, $stateParams, $source,
	$modal, $project, $utils, $localStorage, $q, $map, $sys) {
		...
		$source.$system.query(d).$promise.then(function(resp) {
			...
			// 激活的系统;
			 if ($stateParams.isactive == '1') {
				// proj name ;
				// 是否要 同步;
				promise_B = $source.$system.needSync( {uuids: Object.keys(sys_ref) } ).$promise;
			 }
...
.controller("dastation_add",
	...
	$scope.$watch("systemModel" , function( n, o ){
		...
		$source.$sysProfile.get( {system_model: n.uuid } , function( resp ){
	...
	$scope.commit = function() {
		...
		$source.$system.save( angular.extend( { model: $scope.systemModel.uuid} , $scope.system), function(resp) {
			alert("创建成功!");
