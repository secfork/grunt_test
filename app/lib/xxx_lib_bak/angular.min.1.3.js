...
(function(Q, T, s) {
	...
	function C(b) {
		return function() {
			var a = arguments[0],
				c, a = "[" + (b ? b + ":" : "") + a + "] http://errors.angularjs.org/1.3.0-beta.3/" + (b ? b + "/" : "") + a;
	...
	function ja(b, a, c) {
		var d = b[ib],
			d = Ua[d || -1];
		if (w(c)) d || (b[ib] = d = ++qe, d = Ua[d] = {}), d[a] = c;
		else return d && d[a]
	}
	...
	function qc(b) {
		var a, c;
		"function" ==
		typeof b ? (a = b.$inject) || (a = [], b.length && (c = b.toString().replace(se, ""), c = c.match(te), r(c[1].split(ue), function(b) {
	...
	function we(b, a, c, d) {
		...
		g.onUrlChange = function(a) {
			if (!R) {
				...
				if (d.hashchange) v(b).on("hashchange", h);
				else g.addPollFn(h);
				R = !0
			}
			ba.push(a);
		...
		g.cookies = function(a, b) {
			var d,
				e, f, g;
			if (a) b === s ? m.cookie = escape(a) + "=;path=" + U + ";expires=Thu, 01 Jan 1970 00:00:00 GMT" : A(b) && (d = (m.cookie = escape(a) + "=" + escape(b) + ";path=" + U).length + 1, 4096 < d && c.warn("Cookie '" + a + "' possibly not set or overflowed because it was too large (" + d + " > 4096 bytes)!"));
			else {
				if (m.cookie !== Z)
					for (Z = m.cookie, d = Z.split("; "), M = {}, f = 0; f < d.length; f++) e = d[f], g = e.indexOf("="), 0 < g && (a = unescape(e.substring(0, g)), M[a] === s && (M[a] = unescape(e.substring(g + 1))));
				return M
			}
		};
				...
				d, ["$injector", "$exceptionHandler", function(b, d) {
					...
					r(c[a], function(c, f) {
						try {
							...
							F(g) ? g = {
								compile: Y(g)
							} : !g.compile && g.link && (g.compile = Y(g.link));
		...
		this.$get = ["$injector", "$interpolate", "$exceptionHandler", "$http", "$templateCache", "$parse", "$controller", "$rootScope", "$document", "$sce", "$animate", "$$sanitizeUri", function(a, b, n, q, p, u, I, E, y, H, K, x) {
			...
			function Xa(a, c, d, e, f, g, h, k, p) {
				...
				for (var x = -Number.MAX_VALUE, y, K = p.controllerDirectives, M = p.newIsolateScopeDirective, Z = p.templateDirective, w = p.nonTlbTranscludeDirective, Xa = !1, Ka = p.hasElementTranscludeDirective, L = d.$$element = v(c), G, ga, t, C = e, N, la = 0, P = a.length; la < P; la++) {
					...
					if (t = G.transclude) Xa = !0, G.$$tlb || (Q("transclusion", w, G, L), w = G), "element" == t ? (Ka = !0, x = G.priority, t = B(c, S, V), L = d.$$element = v(T.createComment(" " + ga + ": " + d[ga] + " ")), c = L[0], ob(f, v(wa.call(t, 0)), c), C = ba(t, e, x, g && g.name, {
						nonTlbTranscludeDirective: w
					})) : (t = v(Hb(c)).contents(), L.empty(), C = ba(t, e));
					if (G.template)
			...
			function O(a, b, c, d, e, f, g, h) {
				...
				b.empty();
				q.get(H.getTrustedResourceUrl(E), {
					cache: p
				}).success(function(p) {
					var q, H;
					p = sc(p);
	...
	function ze(b) {
		if (8 >= V && (!b.match(/^(get|post|head|put|delete|options)$/i) ||
	...
	function ee() {
		...
		this.$get = ["$injector", "$exceptionHandler", "$parse", "$browser", function(d, e, f, h) {
			...
			g.prototype = {
				constructor: g,
				$new: function(a) {
					a ? (a = new g, a.$root = this.$root, a.$$asyncQueue = this.$$asyncQueue, a.$$postDigestQueue = this.$$postDigestQueue) : (a = function() {}, a.prototype = this, a =
						new a, a.$id = db());
					a["this"] = a;
					a.$$listeners = {};
					a.$$listenerCount = {};
						...
						a: do {
							if (h = R.$$watchers)
								for (r = h.length; r--;) try {
									if (d = h[r])
										if ((f = d.get(R)) !== (g = d.last) && !(d.eq ? va(f, g) : "number" == typeof f && "number" == typeof g && isNaN(f) && isNaN(g))) x = !0, c = d, d.last = d.eq ? $(f) : f, d.fn(f, g === n ? f : g, R), 5 > s && (w = 4 - s, M[w] || (M[w] = []), t = F(d.exp) ? "fn: " + (d.exp.name || d.exp.toString()) : d.exp, t += "; newVal: " + pa(f) + "; oldVal: " + pa(g), M[w].push(t));
										else if (d === c) {
										x = !1;
										break a
									}
	...
	function id() {
		...
		this.$get = function() {
			return function(c, d) {
				...
				if (!V ||
					8 <= V)
					if (f = qa(c).href, "" !== f && !f.match(e)) return "unsafe:" + f;
				return c
			}
		}
	}
	...
	var dd = /[A-Z]/g,
		...
		Ta = Q.document.removeEventListener ? function(b, a,
			c) {
	...
	r({
		...
		attr: function(b, a, c) {
			...
			if (mb[d])
				...
				else return b[a] || (b.attributes.getNamedItem(a) || z).specified ? d : s;
		...
		readString: function(a) {
			...
			for (var d = "", e = a, f = !1; this.index < this.text.length;) {
				...
				if (f) "u" === h ? (h = this.text.substring(this.index +
					1, this.index + 5), h.match(/[\da-f]{4}/i) || this.throwError("Invalid unicode escape [\\u" + h + "]"), this.index += 4, d += String.fromCharCode(parseInt(h, 16))) : d = (f = Se[h]) ? d + f : d + h, f = !1;
				else if ("\\" === h) f = !0;
				else {
					if (h === a) {
						this.index++;
			...
			return {
				...
				link: function(e, f, h, g, m) {
					...
					e.$watchCollection(g, function(a) {
						...
						h = J.length = C.length;
						for (g = 0; g < h; g++)
							if (z = a === C ? g : C[g], D = a[z], D = n(z, D, g), ya(D, "`track by` id"), H.hasOwnProperty(D)) L = H[D], delete H[D], y[D] =
								L, J[g] = L;
							else {
					...
					function n(e, f, g) {
						function h() {
							...
							for (C = c.length; B < C; B++) {
								...
								F = null;
								E = 0;
								for (D = k.length; E < D; E++) s = k[E], (d = u[E + 1]) ? (F = d.element, d.label !== s.label && F.text(d.label = s.label), d.id !== s.id && F.val(d.id = s.id), d.selected !== s.selected && F.prop("selected", d.selected = s.selected)) : ("" === s.id && z ? K = z : (K = y.clone()).val(s.id).attr("selected", s.selected).text(s.label), u.push({
									element: K,
									label: s.label,
									id: s.id,
									selected: s.selected
